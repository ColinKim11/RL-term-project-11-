...
class PandaPickAndPlaceEnv(RobotTaskEnv):
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=False, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = PickAndPlace(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position, get_fingers_width=robot.get_fingers_width)
        super().__init__(
            robot,
            task,
            render_width=render_width,
...
class PandaStackEnv(RobotTaskEnv):
    ...
    def __init__(
        ...
        sim = PyBullet(render_mode=render_mode, renderer=renderer)
        robot = Panda(sim, block_gripper=False, base_position=np.array([-0.6, 0.0, 0.0]), control_type=control_type)
        task = Stack(sim, reward_type=reward_type, get_ee_position=robot.get_ee_position, get_fingers_width=robot.get_fingers_width, get_ee_velocity=robot.get_ee_velocity)
        super().__init__(
            robot,
            task,
            render_width=render_width,
